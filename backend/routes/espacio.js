...
async function usuarioExiste(usuario_id) {
  ...
  const r = await pool.query('SELECT id FROM usuarios WHERE id = $1', [usuario_id]);
  return r.rows.length > 0;
}

async function registrarPuntosEspacio(usuario_id, herramienta, compartido) {
  try {
    // Máximo una recompensa de Mi espacio por usuario al día.
    // La herramienta puede usarse más veces; solo la primera suma puntos.
...
router.get('/historial', async (req, res) => {
  ...
  try {
    const r = await pool.query(
      `SELECT id, herramienta, estado, mensaje, compartido, creado_en
       FROM espacio_personal_registros
       WHERE usuario_id = $1
    ...
    res.status(500).json({ ok: false, error: 'Error al cargar Mi espacio.' });
  }
});
  ...
  try {
    const insert = await pool.query(
      `INSERT INTO espacio_personal_registros
       ...
       VALUES ($1, $2, $3, $4, $5)
       RETURNING id, herramienta, estado, mensaje, compartido, creado_en`,
    ...
    res.json({
      ...
      mensaje_bonito: puntos > 0
        ? `Herramienta guardada. Cuidarte también cuida el vínculo. +${puntos} pts 🌿`
        : 'Herramienta guardada. Hoy ya se sumaron puntos por Mi espacio, pero igual cuenta.'
    });
  } catch (err) {
    console.error('Error POST /api/espacio/usar:', err);
    res.status(500).json({ ok: false, error: 'Error al guardar herramienta.' });
  }
});
